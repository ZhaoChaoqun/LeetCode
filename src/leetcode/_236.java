...
public class _236 {
    ...
    public static void main(String[] args) {
        ...
        node5.left = node8;
        node5.right = node9;
        System.out.println(new _236().lowestCommonAncestor(node1, node4, node9).val);
    }
    ...
    boolean backtracking(List<TreeNode> stack, TreeNode node, TreeNode target) {
        ...
        if(node.left != null && backtracking(stack, node.left, target))
            return true;
        if(node.right != null && backtracking(stack, node.right, target))
            return true;
