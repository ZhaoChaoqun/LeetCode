...
public class _743 {
    ...
    private void dfs(Integer[][] graphic, int from, int length, int N) {
        for(int i = 1; i <= N; i++)
            if(graphic[from][i] != null && (visited[i] == null || length + graphic[from][i] < visited[i]))
                dfs(graphic, i, visited[i] = length + graphic[from][i], N);
    }
