...
//        System.out.println();
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        dp = new int[M][N];
        // bit：01表示可以去太平洋，10表示可以去大西洋，11表示既可以去太平洋，也可以去大西洋
        for (int i = 0; i < M; ++i) {
            dp[i][0] |= 1;
            dp[i][N-1] |= 2;
        }
    ...
    private void backtracking(int i, int j, int bit) {
        if (i < M - 1 && heights[i+1][j] >= heights[i][j] && (dp[i+1][j] & bit) == 0) {
            dp[i+1][j] |= bit;
            backtracking(i+1, j, bit);
        }
        if (i > 0 && heights[i-1][j] >= heights[i][j] && (dp[i-1][j] & bit) == 0) {
            dp[i-1][j] |= bit;
            backtracking(i - 1, j, bit);
        }
        if (j > 0 && heights[i][j-1] >= heights[i][j] && (dp[i][j-1] & bit) == 0) {
            dp[i][j-1] |= bit;
            backtracking(i, j - 1, bit);
        }
        if (j < N - 1 && heights[i][j+1] >= heights[i][j] && (dp[i][j+1] & bit) == 0) {
            dp[i][j+1] |= bit;
            backtracking(i, j + 1, bit);
        }
    }
