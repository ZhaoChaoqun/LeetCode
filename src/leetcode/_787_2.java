...
public class _787_2 {
    ...
    void backtracking(int level, int[][] flights, int[][] dist, int src, int K, int[] visit) {
        if(level > K)
            return;
        for (int i = 0; i < flights.length; i++) {
            if (flights[i][0] == src && visit[i] == 0) {
                visit[i] = 1;
                int dst = flights[i][1];
                if (level > 0 && dist[dst][level] > dist[src][level-1] + flights[i][2])
                    dist[dst][level] = dist[src][level-1] + flights[i][2];
                backtracking(level + 1, flights, dist, dst, K, visit);
            }
        }
    }
...
//        dist表示到达第i个点，j次中转下的最短距离
        ...
        int[] visit = new int[flights.length];
        for (int i = 0; i < n; i++) {
            for(int j = 0; j <= K; j++)
                dist[i][j] = -1;
        }
        ...
        for (int i = 0; i < flights.length; i++) {
            if (flights[i][0] == src && visit[i] == 0) {
                visit[i] = 1;
                dist[dst][0] = flights[i][2];
                backtracking(1, flights, dist, src, K, visit);
            }
        }
