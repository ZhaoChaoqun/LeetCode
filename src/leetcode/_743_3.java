...
public class _743_3 {
    ...
    public int networkDelayTime(int[][] times, int N, int K) {
        Map<Integer, List<int[]>> graphic = new HashMap<>();
        for(int[] time : times) {
            List<int[]> arc = graphic.getOrDefault(time[0], new ArrayList<>());
            arc.add(time);
            graphic.put(time[0], arc);
        }
    ...
    private Integer[] bfs(Map<Integer, List<int[]>> graphic, int K, int N) {
        ...
        while(!queue.isEmpty()) {
            for(int i = 0, size = queue.size(); i < size; i++) {
                int from = queue.poll();
                if(graphic.containsKey(from))
                    for(int[] arc : graphic.get(from)) {
                        if(visited[arc[1]] == null || visited[arc[1]] > visited[from] + arc[2]) {
                            visited[arc[1]] = visited[from] + arc[2];
                            queue.offer(arc[1]);
                        }
                    }
            }
